...
async function scrapeWebsite(url) {
    const crawler = new PlaywrightCrawler({
        ...
        requestHandler: async ({ page, request, enqueueLinks }) => {
            ...
            await fs.writeFile(path.join(RAW_DIR, 'noah-text.json'), JSON.stringify({ texts: textContent }, null, 2));
            ...
            await fs.writeFile(path.join(RAW_DIR, 'palette.json'), JSON.stringify({ colors: palette }, null, 2));
...
async function runAxeAudit(staticDir) {
...
async function generateEnhancedSite() {
    try {
...
async function runAxeAuditOnEnhanced() {
